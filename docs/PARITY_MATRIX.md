...
| `global.recommendTab` | iOS literal: 45.36pt circle, 30pt lunch bag asset, 1.2pt rim, -14pt lift | Android 직접 지시: bar의 1.5배 geometry는 유지하되 circle 57.834dp, group lift -15.897dp 유지; 36dp 주사위를 Apple 수용 비율(30/45.36, 약 38.25dp)의 풀컬러 `LunchBagNav` 에셋으로 교체(aspect fit, unclipped); 선택/비선택 모두 동일한 white→Ivory 배경과 neutral CaramelDeep 그림자 적용; 라벨은 항상 Ivory 0.7 alpha; 선택 시 하단 빨간 인디케이터 바 표시 | Visual | Android 새 capture comparison required |
...
- release 0.4.1에서 중앙 `추천` 탭의 수기 주사위 글리프를 Apple 수용 비율(30/45.36, 57.834dp 기준 약 38.25dp)의 정품 풀컬러 `LunchBagNav` 에셋으로 교체했다. 중앙 원은 선택/비선택 시 모두 동일한 white→Ivory 그라데이션 배경과 중립 CaramelDeep 그림자(붉은 배경/글로우 제거)를 사용하며, 라벨은 항상 Ivory 0.7 alpha(선택 시 밝기 증가 제거)를 유지한다. 선택 상태 표시는 하단 빨간 인디케이터 바로만 제공하며 기존 선택 시맨틱, 액션, 라벨 문구 및 접근성 레이블은 그대로 보존된다.
